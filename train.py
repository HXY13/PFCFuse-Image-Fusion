...
for epoch in range(num_epochs):
    ...
    for i, (data_VIS, data_IR) in enumerate(loader['train']):
        ...
        if epoch < epoch_gap: #Phase I
            ...
            # loss_dice_V = dice_coeff(data_VIS, data_VIS_hat)
            # loss_dice_I = dice_coeff(data_IR, data_IR_hat)

            loss_rmi_v = relative_diff_loss(data_VIS, data_VIS_hat)
            loss_rmi_i = relative_diff_loss(data_IR, data_IR_hat)
            # print("loss_rmi_v",loss_rmi_v)
            # print("loss_rmi_i", loss_rmi_i)
            # print(coeff_mse_loss_VF * mse_loss_V)
            # loss_cos_v = cosine_similarity(data_VIS, data_VIS_hat)
            # loss_cos_i = cosine_similarity(data_IR, data_IR_hat)
            # print("loss_cos_v", loss_cos_v)
            # print("loss_cos_i", loss_cos_i)
            ...
            # # 加
            # loss_rmi_v = RMI_loss(data_VIS, data_Fuse)
            # loss_rmi_i = RMI_loss(data_IR, data_Fuse)
            # # 加
        ...
        prev_time = time.time()
        sys.stdout.write(
            # "\r[Epoch %d/%d] [Batch %d/%d] [loss: %f] ETA: %.10s Epoch_time: %.10s"
...
if True:
    ...
    torch.save(checkpoint, os.path.join("models/CDDFuse_new_attpool"+timestamp+'.pth'))
